...
def build_layer(X, neuron_num = 1000):
    ...
    with tf.variable_scope("W"):
        W = tf.get_variable(name="W" + str(layer_num), shape=[X.shape[1], neuron_num],
...
def compute_errors(sess, X0, S2, batch_X0, batch_Y, valid_data, valid_target_onehot, test_data, test_target_onehot):
    ...
    return train_error.eval(), valid_error.eval(), test_error.eval()


def compute_losses(sess, loss, X0, Y, X0_train, Y_train, X0_valid, Y_valid, X0_test, Y_test):
    train_loss = sess.run(loss, feed_dict={
        X0: X0_train,
        Y: Y_train
    })
...
def train_no_early_stopping():
    ...
    np_valid_data = valid_data
    np_test_data = test_data
    ...
    with tf.Session() as sess:
        ...
        train_error, valid_error, test_error = compute_errors(sess, X0, S2, batch_X0, batch_Y,
                                                                          valid_data, valid_target_onehot,
                                                                          test_data, test_target_onehot)
        train_loss, valid_loss, test_loss = compute_losses(sess, loss, X0, Y, batch_X0, batch_Y, np_valid_data,
                                                           valid_target, np_test_data, test_target)
        ...
        for epoch in range(num_epochs):
            ...
            train_loss, valid_loss, test_loss = compute_losses(sess, loss, X0, Y, batch_X0, batch_Y,
                                                               np_valid_data, valid_target, np_test_data, test_target)
            ...
            if valid_error_inc > -1:
                if valid_error >= prev_valid_error:
                    valid_error_inc += 1
                else:
                    valid_error_inc = 0
                if valid_error_inc == 5:
                    valid_error_inc = -1
                    with open("part_1_1_3.txt", "a") as file:
                        file.write("\n" + strftime("%Y-%m-%d %H:%M:%S", gmtime()) + "\n")
                        file.write("valid error early stop pt at epoch: " + str(epoch) + "\n")
                        file.write("train error: " + str(train_error) +
                                   " valid error: " + str(valid_error) +
            ...
            if valid_loss_inc > -1:
                ...
                if valid_loss_inc == 5:
                    ...
                    with open("part_1_1_3.txt", "a") as file:
                        file.write("\n" + strftime("%Y-%m-%d %H:%M:%S", gmtime()) + "\n")
                        file.write("valid loss early stop pt at epoch: " + str(epoch) + "\n")
        ...
        print("final valid loss:", valid_loss_list[-1])
        ...
        with open("part_1_1_2.txt", "a") as file:
            ...
            file.write("final valid loss: " + str(valid_loss_list[-1]) + "\n")
            file.write("final test loss: " + str(test_loss_list[-1]) + "\n")

...
def neural_network(pixel_shape, num_hidden_units, num_classifications, weight_decay, learning_rate, n_iter, num_batch, batch_size,
    ...
    #training w/ dropout
    # S1 = tf.nn.dropout(layer_constructor(X0, num_hidden_units),0.5)
    # X1 = tf.nn.relu(S1)
    # S2 = tf.nn.dropout(layer_constructor(X1, num_hidden_units),0.5)
    # X2 = tf.nn.relu(S2)
    # S3 = tf.nn.dropout(layer_constructor(X2, num_classifications),0.5)

    #without dropout
    S1 = layer_constructor(X0, num_hidden_units)
    X1 = tf.nn.dropout(tf.nn.relu(S1), 0.5)
    S2 = layer_constructor(X1, num_hidden_units)
    X2 = tf.nn.dropout(tf.nn.relu(S2), 0.5)
    ...
    with tf.Session() as session:
        ...
        #             weight_array = tf.reshape(W1[:, i], [28, 28])
        #             scale = 255 / session.run(tf.reduce_max(tf.abs(weight_array)))
        #             img = Image.fromarray(scale * session.run(weight_array)).convert('L')
        #             img.save('C:/Users/Elton Wong/PycharmProjects/a3/25/' + str(i + 1) + '.png')
        #
        # for i in range(0, 100):
        #     weight_array = tf.reshape(W1[:, i], [28, 28])
        #     scale = 255 / session.run(tf.reduce_max(tf.abs(weight_array)))
        #     img = Image.fromarray(scale * session.run(weight_array)).convert('L')
        #     img.save('C:/Users/Elton Wong/PycharmProjects/a3/100/' + str(i + 1) + '.png')
...
def main():
    ...
    batch_size = 500
    num_batch = 30
    weight_decay = 0.00186529
    num_hidden_units = 476
    learning_rate = 0.00102265
    pixel_shape = 784
    ...
    print("Final Validation error: ", min(valid_accuracy))
    print("Final test error:", min(test_accuracy))
